...
def getReferencedShapes(
    refShapes: list[tuple[FeatureLike, tuple[str]]],
) -> list[ShapeLike]:
...
def getSelectionEx(hideSelection: bool = False) -> list[tuple[FeatureLike, tuple[str]]]:
    refs = []
...
def newNormalOnShell(shell: ShapeLike, point: Vector) -> Vector:
    ...
    hashToFace = {
        ...
        for i, x in enumerate(shell.Faces, start=1)
    }
    hashToVert = {
        x.hashCode(): VertexDef(x.hashCode(), x, f"Vertex{i}")
        for i, x in enumerate(shell.Vertexes, start=1)
    }
